...
async function resolveAgentTurnAttachments(params) {
	...
	const recentHistoryImages = includeRecentHistoryImages ? resolveRecentInboundHistoryImages({ ctx: params.ctx, limit: 3 }) : []; /* FORGE_RECENT_HISTORY_IMAGE_CAP_3_V1 */
	...
	for (const attachment of currentAttachments.filter(isImageAgentTurnAttachment).slice(0, 3)) currentImageResolved = await resolveImageAttachment(attachment) || currentImageResolved; /* FORGE_CURRENT_IMAGE_CAP_3_V1 */
