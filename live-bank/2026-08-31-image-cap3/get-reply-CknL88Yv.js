...
import { i as getRuntimeConfig } from "./io-C-DnvA4e.js";
...
import "./config-P9o414dV.js";
import "./registry-BUWrOy2m.js";
import { o as resetRegisteredAgentHarnessSessions } from "./registry-DtLZ3rba.js";
import { t as getGlobalHookRunner, u as fireAndForgetHook } from "./hook-runner-global-Cucx8m-W.js";
...
import { P as resolveSessionStoreEntry, R as deriveSessionMetaPatch, V as resolveGroupSessionKey, X as resolveMaintenanceConfigFromInput, b as runExclusiveSessionStoreWrite, et as parseSessionThreadInfoFast } from "./store-CRMOBYMq.js";
...
import { u as commitReplySessionInitialization, v as loadReplySessionInitializationSnapshot, y as loadSessionEntry } from "./session-accessor-B4Nfa9CN.js";
...
import "./model-selection-3MopizeT.js";
import { n as loadModelCatalog } from "./model-catalog-D99EIwtW.js";
...
import { t as hasControlCommand } from "./command-detection-D4nmNgXo.js";
import { n as isInternalSourceReplyChannel } from "./source-reply-delivery-mode-D04x5YKz.js";
...
import { c as resolveSessionResetPolicy, i as resolveThreadFlag, n as resolveChannelResetConfig, o as evaluateSessionFreshness, r as resolveSessionResetType } from "./reset-Bw63W6T_.js";
import { n as resolveSessionKey } from "./session-key-D2ldpjWy.js";
...
import { r as resolveEffectiveAgentRuntime } from "./thinking-runtime-BXaat8AW.js";
...
import "./agent-bundle-mcp-tools-ckWxtWTt.js";
...
import { t as isReasoningTagProvider } from "./provider-utils-DbxZ54d8.js";
...
import { f as extractQueueDirective, m as takeDirectiveToken, p as skipDirectiveArgPrefix, t as resolveQueueSettings } from "./queue-D1uKgXGz.js";
import { n as runAgentHarnessBeforeMessageWriteHook } from "./hook-helpers-CjNtiCqD.js";
import { o as resolvePersistedUserTurnText, r as createUserTurnTranscriptRecorder, t as buildPersistedUserTurnMediaInputsFromFields } from "./user-turn-transcript-B_6cK_uL.js";
...
import { n as forkSessionFromParent, r as resolveParentForkDecision } from "./session-fork-B89RHW3u.js";
...
import { n as resolveEmbeddedFullAccessState } from "./sandbox-info-Bt_MtfWP.js";
...
import { t as resolveDefaultModel } from "./directive-handling.defaults-CuhVHEiP.js";
...
import { n as shouldHandleTextCommands } from "./commands-text-routing-BpBVSQuy.js";
import "./commands-registry-Bzez3soi.js";
import { c as takeCommandSessionMetadataChangesFromTargets } from "./commands-goal-CbZMO-wY.js";
import { i as resolveConversationBindingContextFromMessage } from "./conversation-binding-input-C55GavUy.js";
...
import { t as resolveRoutedDeliveryThreadId } from "./routed-delivery-thread-CV-DvtsC.js";
import { n as resolveStoredModelOverride, t as isStaleHeartbeatAutoFallbackOverride } from "./stored-model-override-Drz7_BJI.js";
...
import { t as resolveEffectiveResetTargetSessionKey } from "./acp-reset-target-DicUg2cf.js";
...
import { n as clearSessionResetRuntimeState, t as resolveResetPreservedSelection } from "./reset-preserved-selection-D1k6QV_3.js";
import { a as resolveEnvelopeFormatOptions } from "./envelope-CXHEh-mU.js";
import { r as updateAmbientTranscriptWatermark } from "./ambient-transcript-watermark-DFoUtI4U.js";
...
import { t as resolveModelSelectionFromDirective } from "./directive-handling.model-selection-Dh2IHsy7.js";
import { n as resolveSessionAuthProfileOverride } from "./session-override-v0C6eiL6.js";
import { n as createModelSelectionState, r as resolveContextTokens, t as createFastTestModelSelectionState } from "./model-selection-BoqmpZ-L.js";
...
import { i as resolveSkillCommandInvocation, r as listReservedChatSlashCommandNames } from "./chat-commands-CPuqy5Qg.js";
import { t as persistReplySessionEntry } from "./session-entry-persistence-qS_1xTqH.js";
import { n as consumeSessionSkillSuggestion } from "./skill-suggestions-DGZpW-LY.js";
import { a as buildInboundUserContextPrefix, c as resolveInboundUserContextPromptJoiner, d as resolveSilentReplySettings, i as buildInboundMetaSystemPrompt, l as resolveCurrentTurnImages, n as resolveTypingMode, o as formatActiveGoalContext, r as resolveActiveRunQueueAction } from "./typing-mode-BfZebQdd.js";
import { n as resolvePreparedReplyQueueState } from "./get-reply-run-queue-CJI8xCZG.js";
import { n as resolveEffectiveToolInventory } from "./tools-effective-inventory-CrNIRVF3.js";
import { t as drainFormattedSystemEvents } from "./session-system-events-DWqcnFIu.js";
...
const directiveImplLoader = createLazyImportLoader(() => import("./directive-handling.impl-CEt8Kruu.js"));
const directiveFastLaneLoader = createLazyImportLoader(() => import("./directive-handling.fast-lane-kcmjO0IF.js"));
...
const skillToolDispatchRuntimeLoader = createLazyImportLoader(() => import("./tool-dispatch-3ch2VPYF.js"));
	...
	if (skillInvocation) {
		...
		if (dispatch?.kind === "tool") {
			...
			if (!tool) {
				...
				return {
					kind: "reply",
					reply: markCommandReplyForDelivery({ text: `âŒ Tool not available: ${dispatch.toolName}` })
				};
			}
			...
			try {
				...
				if (blockedReason) {
					...
					return {
						kind: "reply",
						reply: markCommandReplyForDelivery({ text: `âŒ Tool call blocked: ${blockedReason}` })
					};
				}
				...
				return {
					kind: "reply",
					reply: markCommandReplyForDelivery({ text: `âŒ ${message}` })
				};
			}
		}
...
const statusCommandRuntimeLoader = createLazyImportLoader(() => import("./commands-status-CreYyTZ6.js"));
...
const sessionAccessorRuntimeLoader = createLazyImportLoader(() => import("./session-accessor-CYjlXwGi.js"));
...
function buildInboundMediaNote(ctx) {
    ...
    const visibleEntries = entries.filter((entry) => {
        const mediaType = normalizeLowercaseStringOrEmpty(entry.type);
        const mediaPath = normalizeLowercaseStringOrEmpty(entry.path);
        const mediaUrl = normalizeLowercaseStringOrEmpty(entry.url);
        const isImage = mediaType.startsWith("image/") || /\.(?:avif|bmp|gif|heic|heif|jpe?g|png|tiff?|webp)(?:$|[?#])/i.test(mediaPath) || /\.(?:avif|bmp|gif|heic|heif|jpe?g|png|tiff?|webp)(?:$|[?#])/i.test(mediaUrl);
        if (!isImage) return true;
        discoveredImageCount += 1;
        return discoveredImageCount <= 3;
    });
    const omittedImageCount = Math.max(0, discoveredImageCount - 3);
    const lines = [`[media attached: ${count} files]`];
    for (const entry of visibleEntries) lines.push(formatMediaAttachedLine({
        path: entry.path,
        index: entry.index + 1,
        total: count,
        type: entry.type,
    ...
    if (omittedImageCount > 0) lines.push(`[${omittedImageCount} additional image${omittedImageCount === 1 ? "" : "s"} omitted from inspection]`); /* FORGE_MEDIA_PATH_IMAGE_CAP_3_V1 */
    return lines.join("\n");
}
//#endregion
...
const loadDeliverRuntime = createLazyPromiseLoader(() => import("./runtime-9mF07qSz.js"), { cacheRejections: true }).load;
...
function buildWarningText(warning) {
	...
	return `âš ï¸ Session maintenance warning: this active session would be evicted (${reasons.length > 0 ? reasons.join(" and ") : "over maintenance limits"}). Maintenance is set to warn-only, so nothing was reset. To enforce cleanup, set \`session.maintenance.mode: "enforce"\` or increase the limits.`;
}
...
async function prepareReplySessionParentFork(params) {
	...
	if (decision.status === "skip") {
		params.warn(`skipping parent fork (parent too large): parentKey=${params.parentSessionKey} â†’ sessionKey=${params.sessionKey} parentTokens=${decision.parentTokens} maxTokens=${decision.maxTokens}`);
		return {
			...params.sessionEntry,
	...
	params.warn(`forking from parent session: parentKey=${params.parentSessionKey} â†’ sessionKey=${params.sessionKey} parentTokens=${decision.parentTokens ?? "unknown"}`);
		...
		if (opts?.isHeartbeat) {
			if (classifyHeartbeatPendingFinalDelivery(text, resolveHeartbeatAckMaxChars(cfg, agentId)).shouldClear) {
				...
				if (sessionKey && storePath) {
					const { updateSessionEntry } = await import("./session-accessor-CYjlXwGi.js");
