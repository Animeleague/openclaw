import { appendFileSync as forgeCodexTurnDiagAppendFileSync } from "node:fs"; // FORGE_CODEX_TURN_PAYLOAD_DIAG_V1
...
import { CODEX_APP_SERVER_CONTEXT_ENGINE_HOST, FAST_MODE_AUTO_PROGRESS_KIND, assembleHarnessContextEngine, assertContextEngineHostSupport, awaitAgentEndSideEffects, bootstrapHarnessContextEngine, buildAgentHarnessUserInputAnswers, buildBootstrapContextForFiles, buildHarnessContextEngineRuntimeContext, buildHarnessContextEngineRuntimeContextFromUsage, clearActiveEmbeddedRun, deliverAgentHarnessUserInputPrompt, embeddedAgentLog, emitAgentEvent, emptyAgentHarnessUserInputAnswers, finalizeHarnessContextEngineTurn, formatErrorMessage, formatFastModeAutoProgressText, getAgentHarnessHookRunner, getBeforeToolCallPolicyDiagnosticState, isActiveHarnessContextEngine, loadCodexBundleMcpThreadConfig, resolveAgentHarnessBeforePromptBuildResult, resolveAgentRunAbortLifecycleFields, resolveBootstrapFilesForRun, resolveContextEngineOwnerPluginId, resolveFastModeForElapsed, resolveSandboxContext, resolveSessionAgentIds as resolveSessionAgentIds$1, resolveUserPath, runAgentCleanupStep, runAgentEndSideEffects, runAgentHarnessLlmInputHook, runAgentHarnessLlmOutputHook, runHarnessContextEngineMaintenance, setActiveEmbeddedRun, supportsModelTools } from "openclaw/plugin-sdk/agent-harness-runtime";
...
async function buildCodexWorkspaceBootstrapContext(params) {
	try {
		...
		const forgeMemoriesPath = path.join(params.resolvedWorkspace, "MEMORIES.md");
		let forgeMemoriesInstructions;
		try {
			const forgeMemoriesContent = await fs$1.readFile(forgeMemoriesPath, "utf8");
			if (forgeMemoriesContent.trim()) forgeMemoriesInstructions = [
				"## Forge Long-Term Memory",
				"",
				"### MEMORIES.md",
				"",
				forgeMemoriesContent
			].join("\n").trim();
		} catch (error) {
			embeddedAgentLog.warn("failed to load Forge MEMORIES.md", { error });
		}
		...
		return {
			...
			developerInstructions: joinPresentSections(renderCodexWorkspaceThreadDeveloperInstructions(developerInstructionFiles), forgeMemoriesInstructions),
...
function renderCodexSkillsCollaborationInstructions(params) {
	...
	if (!prompt) return;
	const allowed = /* @__PURE__ */ new Set(["browser-automation", "discord"]);
	const blocks = Array.from(prompt.matchAll(/<skill>[\s\S]*?<\/skill>/gi)).map((match) => match[0] ?? "").filter((block) => {
		const name = block.match(/<name>\s*([^<]+?)\s*<\/name>/i)?.[1]?.trim();
		return name && allowed.has(name);
	});
	if (blocks.length === 0) return;
	return ["## OpenClaw Skills", "", "<available_skills>", ...blocks, "</available_skills>"].join("\n");
}
/**
...
async function rotateOversizedCodexAppServerStartupBinding(params) {
	const binding = params.binding;
        ...
        // FORGE_CODEX_NATIVE_ROLLOVER_BOOTSTRAP_CONSUME_V4
        if (
                binding.contextEngine?.engineId === "forge-native-rollover" &&
                binding.contextEngine?.projection?.mode === "thread_bootstrap"
        ) {
                embeddedAgentLog.info(
                        "forge consuming intentional native rollover bootstrap; starting clean thread",
                        {
	...
	const reserveTokens = Math.min(resolveCodexAppServerNativeThreadReserveTokens(params.config), 8e3); // FORGE_CODEX_NATIVE_AUTOCOMPACT_HEADROOM_V1
...
async function runCodexAppServerAttempt(params, options) {
	...
	let bindingIdentity = sessionBindingIdentity({ // FORGE_MONITOR_BINDING_IDENTITY_MUTABLE_V1
		sessionId: params.sessionId,
		sessionKey: params.sessionKey,
		agentId: params.agentId,
		config: params.config
	...
	const forgeNativeMonitorBindingStateV104 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_MONITOR_WARM_ROLLBACK_V1_04_FINAL5
	const forgeNativeMonitorBindingEntryV104 = forgeNativeMonitorBindingStateV104?.disposableRuns instanceof Map ? forgeNativeMonitorBindingStateV104.disposableRuns.get(params.runId) : void 0;
	if (forgeNativeMonitorBindingEntryV104?.reason === "forge-monitor" && bindingIdentity.kind === "session") {
		const forgeNativeMonitorLogicalIdentityV104 = bindingIdentity;
		const forgeNativeMonitorModelV104 = (params.modelId ?? "unknown").trim().toLowerCase().replace(/[^a-z0-9._-]+/g, "_");
		const forgeNativeMonitorPhysicalIdentityV104 = {
			kind: "session",
			agentId: forgeNativeMonitorLogicalIdentityV104.agentId,
			sessionId: `${forgeNativeMonitorLogicalIdentityV104.sessionId}:forge-monitor-final9:${forgeNativeMonitorModelV104}`
		};
		bindingStore = scopeCodexRunBindingStore({
			bindingStore,
			logicalIdentity: forgeNativeMonitorLogicalIdentityV104,
			physicalIdentity: forgeNativeMonitorPhysicalIdentityV104
		});
		bindingIdentity = forgeNativeMonitorPhysicalIdentityV104;
	}
	...
	const forgeNativeThreadRollbackStartupV1 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_NATIVE_THREAD_ROLLBACK_V1
	if (forgeNativeThreadRollbackStartupV1?.rollbacks instanceof Map) {
		const forgeStartupSidecarsV1 = globalThis.__forgeCodexLunaSidecarsV2;
		const forgeStartupSidecarKeyV1 = JSON.stringify(bindingIdentity);
		const forgeStartupSidecarV1 = forgeStartupSidecarsV1?.get(forgeStartupSidecarKeyV1);
		const forgeStartupRequestedModelV1 = (params.modelId ?? "").trim().toLowerCase().split("/").at(-1);
		const forgeStartupActiveBindingV1 = forgeStartupRequestedModelV1 === "gpt-5.6-luna" && forgeStartupSidecarV1?.threadId ? forgeStartupSidecarV1 : startupBinding;
		const forgeStaleRollbackV1 = forgeStartupActiveBindingV1?.threadId ? forgeNativeThreadRollbackStartupV1.rollbacks.get(forgeStartupActiveBindingV1.threadId) : void 0;
		if (forgeStaleRollbackV1?.forkThreadId && forgeStaleRollbackV1?.originalBinding) {
			let forgeStaleRollbackRestoredV1 = false;
			if (forgeStaleRollbackV1.isLunaSidecar && forgeStartupSidecarV1?.threadId === forgeStartupActiveBindingV1.threadId) {
				forgeStartupSidecarsV1.set(forgeStartupSidecarKeyV1, { ...forgeStaleRollbackV1.originalBinding, threadId: forgeStaleRollbackV1.forkThreadId });
				forgeStaleRollbackRestoredV1 = true;
			} else if (!forgeStaleRollbackV1.isLunaSidecar && startupBinding?.threadId === forgeStartupActiveBindingV1.threadId) {
				forgeStaleRollbackRestoredV1 = await bindingStore.mutate(bindingIdentity, {
					kind: "set",
					binding: { ...forgeStaleRollbackV1.originalBinding, threadId: forgeStaleRollbackV1.forkThreadId }
				});
				if (forgeStaleRollbackRestoredV1) startupBinding = { ...forgeStaleRollbackV1.originalBinding, threadId: forgeStaleRollbackV1.forkThreadId };
			}
			if (forgeStaleRollbackRestoredV1) {
				forgeNativeThreadRollbackStartupV1.rollbacks.delete(forgeStartupActiveBindingV1.threadId);
				forgeNativeThreadRollbackStartupV1.snapshotAttemptedThreadIds?.delete(forgeStartupActiveBindingV1.threadId);
				embeddedAgentLog.info("forge native thread rollback restored a clean pre-tool fork from a previous incomplete tool turn", {
					dirtyThreadId: forgeStartupActiveBindingV1.threadId,
					cleanThreadId: forgeStaleRollbackV1.forkThreadId
				});
			}
		}
	}
	...
	const toolBridge = createCodexDynamicToolBridge({
		...
		registeredTools: await buildDynamicTools({
			params: dynamicToolParams.senderIsOwner === true && (dynamicToolParams.images?.length ?? 0) === 0 ? dynamicToolParams : { ...dynamicToolParams, senderIsOwner: true, images: void 0 }, // FORGE_CODEX_DURABLE_REGISTRATION_PROOF_V1
			resolvedWorkspace,
			effectiveWorkspace,
			effectiveCwd,
			sandboxSessionKey,
	...
	const applyActiveContextEngineProjection = async (decisionStartupBinding) => {
		...
		const projectionDecision = contextEngineProjection ? resolveContextEngineBootstrapProjectionDecision({
			...
			dynamicToolsFingerprint: codexDynamicToolsFingerprint(forgeStableToolCatalogV2),
			legacyDynamicToolsFingerprint: codexLegacyDynamicToolsFingerprint(forgeStableToolCatalogV2)
		}) : {
			project: true,
	...
	const forgeRequestedModelForContinuityV2 = (params.modelId ?? "").trim().toLowerCase().split("/").at(-1);
	const forgeLunaContinuitySidecarV2 = forgeLunaSidecarsForContinuityV2?.get(JSON.stringify(bindingIdentity));
	const forgeContinuityBindingV2 = forgeRequestedModelForContinuityV2 === "gpt-5.6-luna" && forgeLunaContinuitySidecarV2?.threadId ? forgeLunaContinuitySidecarV2 : startupBinding;
	if (precomputeNoContextEngineStaleBindingProjection(forgeContinuityBindingV2)) await rebuildCodexPromptBuildFromCurrentProjection();
	...
	try {
		...
		const forgeNativeThreadRollbackPrestartV1 = globalThis[Symbol.for("forge.native-thread-rollback.v1")] ??= {}; // FORGE_CODEX_NATIVE_THREAD_ROLLBACK_V1
		if (!(forgeNativeThreadRollbackPrestartV1.disposableRuns instanceof Map)) forgeNativeThreadRollbackPrestartV1.disposableRuns = new Map();
		if (!(forgeNativeThreadRollbackPrestartV1.disposableThreadIds instanceof Set)) forgeNativeThreadRollbackPrestartV1.disposableThreadIds = new Set();
		if (!(forgeNativeThreadRollbackPrestartV1.rollbacks instanceof Map)) forgeNativeThreadRollbackPrestartV1.rollbacks = new Map();
		if (!(forgeNativeThreadRollbackPrestartV1.snapshotAttemptedThreadIds instanceof Set)) forgeNativeThreadRollbackPrestartV1.snapshotAttemptedThreadIds = new Set();
		if (typeof params.prompt === "string" && params.prompt.includes("<transient_tool_context>")) {
			forgeNativeThreadRollbackPrestartV1.disposableRuns.set(params.runId, { at: Date.now(), reason: "transient-tool-context" });
		}
		while (forgeNativeThreadRollbackPrestartV1.disposableRuns.size > 512) forgeNativeThreadRollbackPrestartV1.disposableRuns.delete(forgeNativeThreadRollbackPrestartV1.disposableRuns.keys().next().value);
		while (forgeNativeThreadRollbackPrestartV1.rollbacks.size > 512) forgeNativeThreadRollbackPrestartV1.rollbacks.delete(forgeNativeThreadRollbackPrestartV1.rollbacks.keys().next().value);
		while (forgeNativeThreadRollbackPrestartV1.snapshotAttemptedThreadIds.size > 512) forgeNativeThreadRollbackPrestartV1.snapshotAttemptedThreadIds.delete(forgeNativeThreadRollbackPrestartV1.snapshotAttemptedThreadIds.values().next().value);
		const startupResult = await startCodexAttemptThread({
			...
			effectiveWorkspace,
			effectiveCwd,
			dynamicTools: forgeStableToolCatalogV2,
	...
	const forgeNativeThreadRollbackProjectionV102 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_MONITOR_PROJECTION_BYPASS_V1_02
    const forgeMonitorWarmProjectionV104 = forgeNativeThreadRollbackProjectionV102?.disposableRuns instanceof Map && forgeNativeThreadRollbackProjectionV102.disposableRuns.get(params.runId)?.reason === "forge-monitor"; // FORGE_CODEX_MONITOR_WARM_ROLLBACK_V1_04_FINAL5
	...
	let completed = false;
        ...
        // FORGE_CODEX_TRANSIENT_TOOL_TRANSACTION_V3
        // Exact tool-bearing native turn for this run-attempt closure.
        let forgeTransientToolTurnV3;

        // FORGE_CODEX_TRANSIENT_TOOL_FINAL_PASS1_DIAG_V1
        const forgeTransientToolPass1DiagV1 = (event, data = {}) => {
                try {
                        const home = process.env.USERPROFILE || process.env.HOME;
                        if (!home) return;

                        forgeCodexTurnDiagAppendFileSync(
                                `${home}\\.openclaw\\logs\\forge-transient-tool-pass1.jsonl`,
                                `${JSON.stringify({
                                        ts: new Date().toISOString(),
                                        event,
                                        ...data
                        ...
                        // Diagnostics must never affect the turn.
                }
        };
	...
	const noteNotificationReceived = (notification, scope, receivedAtMs) => {
		...
		if (scope.turnId === turnId) {
			...
			if (modelToolCallId) {
                                ...
                                // FORGE_CODEX_TRANSIENT_TOOL_NATIVE_ARM_FINAL_V1
                                // Native Codex tools such as exec are observed here,
                                // not through the OpenClaw dynamic-tool request path.
                                forgeTransientToolTurnV3 = {
                                        threadId: thread.threadId,
                                        turnId
                                };

                                forgeTransientToolPass1DiagV1("native-tool-armed", {
                                        runId: params.runId,
                                        threadId: thread.threadId,
                                        toolTurnId: turnId,
                                        modelToolCallId
...
const forgeTransientToolTxnRunStateV2 =
globalThis[Symbol.for("forge.transient-tool-transaction.v2")] ??=
{ dirtyRunIds: new Set() }; // FORGE_CODEX_TRANSIENT_TOOL_TRANSACTION_V2

forgeTransientToolTxnRunStateV2.dirtyRunIds.add(params.runId);
...
forgeTransientToolPass1DiagV1("tool-armed", {
runId: params.runId,
threadId: thread.threadId,
toolTurnId: call.turnId
});

embeddedAgentLog.info(
"forge transient tool transaction v3 armed",
{
threadId: thread.threadId,
toolTurnId: call.turnId,
runId: params.runId
			...
			const forgeNativeThreadRollbackToolV1 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_NATIVE_THREAD_ROLLBACK_V1
			const forgeNativeMonitorToolBypassV104 = forgeNativeThreadRollbackToolV1?.disposableRuns instanceof Map && forgeNativeThreadRollbackToolV1.disposableRuns.get(params.runId)?.reason === "forge-monitor"; // FORGE_CODEX_MONITOR_WARM_ROLLBACK_V1_04_FINAL5
			if (false && forgeNativeThreadRollbackToolV1 && !forgeNativeThreadRollbackToolV1.disposableThreadIds?.has(thread.threadId) && !forgeNativeThreadRollbackToolV1.snapshotAttemptedThreadIds?.has(thread.threadId)) { // FORGE_CODEX_TRANSIENT_TOOL_TRANSACTION_V1_OLD_FORK_DISABLED
				forgeNativeThreadRollbackToolV1.snapshotAttemptedThreadIds?.add(thread.threadId);
				try {
					const forgeToolSidecarsV1 = globalThis.__forgeCodexLunaSidecarsV2;
					const forgeToolSidecarKeyV1 = JSON.stringify(bindingIdentity);
					const forgeToolSidecarV1 = forgeToolSidecarsV1?.get(forgeToolSidecarKeyV1);
					const forgeToolDurableBindingV1 = await bindingStore.read(bindingIdentity);
					const forgeToolIsLunaSidecarV1 = forgeToolSidecarV1?.threadId === thread.threadId;
					const forgeToolActiveBindingV1 = forgeToolIsLunaSidecarV1 ? forgeToolSidecarV1 : forgeToolDurableBindingV1?.threadId === thread.threadId ? forgeToolDurableBindingV1 : void 0;
					if (forgeToolActiveBindingV1?.threadId) {
						const forgeForkResponseV1 = await client.request("thread/fork", {
							threadId: thread.threadId,
							beforeTurnId: turnId
						}, { signal: runAbortController.signal });
						const forgeForkThreadIdV1 = forgeForkResponseV1 && typeof forgeForkResponseV1 === "object" && forgeForkResponseV1.thread && typeof forgeForkResponseV1.thread.id === "string" && forgeForkResponseV1.thread.id.trim() ? forgeForkResponseV1.thread.id : void 0;
						if (forgeForkThreadIdV1) {
							forgeNativeThreadRollbackToolV1.rollbacks.set(thread.threadId, {
								forkThreadId: forgeForkThreadIdV1,
								originalBinding: { ...forgeToolActiveBindingV1 },
								isLunaSidecar: forgeToolIsLunaSidecarV1,
								runId: params.runId,
								turnId,
								at: Date.now()
							});
					...
					embeddedAgentLog.warn("forge native thread rollback snapshot failed safely; continuing the tool turn on the current thread", {
						threadId: thread.threadId,
						turnId,
						error: formatErrorMessage(error)
					});
	...
	const startCodexTurn = async () => {
		 ...
		 * Route Forge/OpenClaw current-turn support context through
		 * turn-scoped developer instructions instead of native user history.
		 */
		const forgeTransientRuntimeCarrierV4 = (() => {
		        if (params.messageProvider !== "discord") return;
		
		        const source = codexTurnPromptText;
		        const marker = "\n[meta ";
		
		        let durableStart = source.lastIndexOf(marker);
		        if (durableStart >= 0) durableStart += 1;
		        else if (source.startsWith("[meta ")) durableStart = 0;
		        else return;
		
		        let transientText = source.slice(0, durableStart).trim();
		        const durablePromptText = source.slice(durableStart).trimStart();
		
		        transientText = transientText
		                .replace(/\n*Current user request:\s*$/i, "")
		                .trim();
		
		        if (!transientText || !durablePromptText) return;
		        ...
		        embeddedAgentLog.info("forge transient runtime context routed through turn-scoped developer instructions", {
		                sessionId: params.sessionId,
		                transientChars: transientText.length,
		                durablePromptChars: durablePromptText.length
		        });
		...
		const turnStartParams = buildTurnStartParams(params, {
			...
			promptText: forgeTransientRuntimeCarrierV4?.promptText ?? codexTurnPromptText,
	...
	if (thread.lifecycle.action === "resumed" && (thread.lifecycle.activeTurnIds?.length ?? 0) > 0) {
		embeddedAgentLog.info("codex app-server resumed thread has active native turn; waiting before turn/start", { threadId: thread.threadId });
		emitCodexAppServerEvent(params, {
			stream: "codex_app_server.lifecycle",
			data: {
		...
		if (!nativeTurnCompleted && !runAbortController.signal.aborted) embeddedAgentLog.warn("codex app-server active native turn did not complete before turn/start wait timed out", { threadId: thread.threadId });
	}
	try {
		...
		if (turn === void 0 && shouldUseFreshCodexThreadAfterContextEngineOverflow({
			...
			try {
				if (!await bindingStore.mutate(bindingIdentity, {
					kind: "clear",
					threadId: thread.threadId
				})) embeddedAgentLog.warn("codex app-server preserved newer context-engine binding after resume overflow; skipping fresh retry", {
					threadId: thread.threadId,
					error: formatErrorMessage(turnStartError)
				});
				else {
	...
	try {
		...
		shouldDelayNativeHookRelayUnregister = completedTurnStatus === "completed" && !effectiveTimedOut && !runAbortController.signal.aborted && !finalAborted && !finalPromptError;
                forgeTransientToolPass1DiagV1("pre-finalizer", {
                        runId: params.runId,
                        threadId: thread.threadId,
                        activeTurnId,
                        toolThreadId: forgeTransientToolTurnV3?.threadId,
                        toolTurnId: forgeTransientToolTurnV3?.turnId,
                        completedTurnStatus,
                        effectiveTimedOut,
                        runAborted: runAbortController.signal.aborted,
                        finalAborted,
                        hasFinalPromptError: Boolean(finalPromptError),
                        shouldDelayNativeHookRelayUnregister,
                        disposableReason:
                                globalThis[Symbol.for("forge.native-thread-rollback.v1")]?.disposableRuns instanceof Map
                                        ? globalThis[Symbol.for("forge.native-thread-rollback.v1")].disposableRuns.get(params.runId)?.reason
                                        : void 0
                });

                if (shouldDelayNativeHookRelayUnregister) try {
                        const forgeTransientToolTxnStateV1 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_TRANSIENT_TOOL_TRANSACTION_V1
                        const forgeTransientToolTxnRunStateV2 =
                                globalThis[Symbol.for("forge.transient-tool-transaction.v2")] ??=
                                { dirtyRunIds: new Set() }; // FORGE_CODEX_TRANSIENT_TOOL_TRANSACTION_V2
                        const forgeTransientToolTxnMonitorV1 =
                                forgeTransientToolTxnStateV1?.disposableRuns instanceof Map &&
                                forgeTransientToolTxnStateV1.disposableRuns.get(params.runId)?.reason === "forge-monitor";

                        embeddedAgentLog.info(
                                "forge transient tool transaction v3 finalizer state",
                                {
                                        threadId: thread.threadId,
                                        activeTurnId,
                                        toolThreadId: forgeTransientToolTurnV3?.threadId,
                                        toolTurnId: forgeTransientToolTurnV3?.turnId,
                                        monitor: forgeTransientToolTxnMonitorV1,
                                        completedTurnStatus
                                }
                        );
                        ...
                        if (
                                ...
                                /*
                                 * FORGE_CODEX_TRANSIENT_TOOL_TRANSACTION_V1
                                 *
                                 * A successful ordinary tool-bearing turn is transactional:
                                 *
                                 * 1. Read back the completed native turn.
                                 * 2. Preserve only the visible user request + final assistant answer.
                                 * 3. Roll back the whole tool-bearing native turn.
                                 * 4. Inject the clean visible pair without starting another model turn.
                                 *
                                 * The native thread ID never changes, so Codex keeps the same
                                 * prompt-cache affinity while raw tool calls/results disappear
                                 * from future model-visible native history.
                                 */

                                forgeTransientToolPass1DiagV1("txn-enter", {
                                        runId: params.runId,
                                        threadId: thread.threadId,
                                        activeTurnId,
                                        toolTurnId: forgeTransientToolTurnV3?.turnId
                                ...
                                const forgeTxnReadV1 = await client.request(
                                        "thread/read",
                                        {
                                                threadId: thread.threadId,
                                                includeTurns: true
                                ...
                                forgeTransientToolPass1DiagV1("thread-read-returned", {
                                        runId: params.runId,
                                        threadId: thread.threadId,
                                        hasThread: Boolean(forgeTxnReadV1?.thread),
                                        turnCount: Array.isArray(forgeTxnReadV1?.thread?.turns)
                                                ? forgeTxnReadV1.thread.turns.length
                                                : -1
                                });

                                const forgeTxnTurnsV1 =
                                        Array.isArray(forgeTxnReadV1?.thread?.turns)
                                                ? forgeTxnReadV1.thread.turns
                                                : [];

                                const forgeTxnTurnV1 =
                                        forgeTxnTurnsV1.find(
                                                (entry) => entry?.id === forgeTransientToolTurnV3.turnId
                                        ) ??
                                        forgeTxnTurnsV1.at(-1);

                                const forgeTxnItemsV1 =
                                        Array.isArray(forgeTxnTurnV1?.items)
                                                ? forgeTxnTurnV1.items
                                                : [];

                                const forgeTxnItemTextV1 = (item) => {
                                        if (
                                ...
                                const forgeTxnUserItemV1 =
                                        forgeTxnItemsV1.find((item) => {
                                                const type = String(
                                                        item?.type ?? ""
                                                ).toLowerCase();
                                ...
                                if (
                                        !forgeTxnUserTextV1 ||
                                        !forgeTxnAssistantTextV1
                                ) {
                                        throw new Error(
                                                "forge transient tool transaction could not recover clean visible user/assistant pair"
                                        );
                                }
                                ...
                                embeddedAgentLog.info(
                                        "forge transient tool transaction committed clean visible history on same native thread",
                                        {
                                                ...
                                                threadId: thread.threadId,
                                                turnId: activeTurnId,
                                                userChars: forgeTxnUserTextV1.length,
                                                assistantChars: forgeTxnAssistantTextV1.length
                                        }
                                );
                        }

                        forgeTransientToolTurnV3 = void 0;
                        forgeTransientToolTxnRunStateV2.dirtyRunIds.delete(params.runId);
		...
		await runAgentCleanupStep({
			...
			cleanup: async () => {
				await trajectoryRecorder?.flush();
	const forgeNativeMonitorCleanupStateV109 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_MONITOR_CLEANUP_ROLLBACK_V1_04_FINAL9
	const forgeNativeMonitorCleanupEntryV109 = forgeNativeMonitorCleanupStateV109?.disposableRuns instanceof Map ? forgeNativeMonitorCleanupStateV109.disposableRuns.get(params.runId) : void 0;
	if (forgeNativeMonitorCleanupEntryV109?.reason === "forge-monitor") {
		let forgeNativeMonitorCleanupRollbackKeptV109 = false;
		try {
			if (terminalState.turnSucceeded) {
				await resourceState.client.request("thread/rollback", {
					threadId: resourceState.thread.threadId,
					numTurns: 1
				}, { signal: runAbortController.signal });
				forgeNativeMonitorCleanupRollbackKeptV109 = true;
				resourceState.thread.preserveNativeModel = true; // FORGE_CODEX_MONITOR_RELEASE_LIVE_AFTER_ROLLBACK_FINAL10
			}
		} catch {
			// Fail closed below: retire only the private monitor binding.
		} finally {
			if (!forgeNativeMonitorCleanupRollbackKeptV109) {
				try {
					await bindingStore.mutate(bindingIdentity, {
						kind: "clear",
						threadId: resourceState.thread.threadId
					...
					// Cleanup must never turn a completed visible reply into failure.
				}
			}
...
async function markCodexAppServerBindingCoveredThroughTurn(params) {
	const forgeNativeThreadRollbackCoverageV1 = globalThis[Symbol.for("forge.native-thread-rollback.v1")]; // FORGE_CODEX_NATIVE_THREAD_ROLLBACK_V1
	if (forgeNativeThreadRollbackCoverageV1?.disposableThreadIds?.has(params.threadId)) {
		forgeNativeThreadRollbackCoverageV1.disposableThreadIds.delete(params.threadId);
		forgeNativeThreadRollbackCoverageV1.snapshotAttemptedThreadIds?.delete(params.threadId);
		forgeNativeThreadRollbackCoverageV1.rollbacks?.delete(params.threadId);
		return;
	}
	const forgeRollbackV1 = forgeNativeThreadRollbackCoverageV1?.rollbacks?.get(params.threadId);
	if (forgeRollbackV1?.forkThreadId && forgeRollbackV1?.originalBinding) {
		const forgeLunaSidecarsV2 = globalThis.__forgeCodexLunaSidecarsV2;
		const forgeLunaSidecarKeyV2 = JSON.stringify(params.identity);
		const forgeLunaSidecarV2 = forgeLunaSidecarsV2?.get(forgeLunaSidecarKeyV2);
		let forgeRollbackRestoredV1 = false;
		if (forgeRollbackV1.isLunaSidecar && forgeLunaSidecarV2?.threadId === params.threadId) {
			forgeLunaSidecarsV2.set(forgeLunaSidecarKeyV2, { ...forgeRollbackV1.originalBinding, threadId: forgeRollbackV1.forkThreadId });
			forgeRollbackRestoredV1 = true;
		} else if (!forgeRollbackV1.isLunaSidecar) {
			await params.bindingStore.withLease(params.identity, async () => {
				const forgeCurrentBindingV1 = await params.bindingStore.read(params.identity);
				if (forgeCurrentBindingV1?.threadId !== params.threadId) {
					embeddedAgentLog.warn("forge native thread rollback found a newer binding; preserving it instead of overwriting", {
						dirtyThreadId: params.threadId,
						boundThreadId: forgeCurrentBindingV1?.threadId
					});
					return;
				}
				forgeRollbackRestoredV1 = await params.bindingStore.mutate(params.identity, {
					kind: "set",
					binding: { ...forgeRollbackV1.originalBinding, threadId: forgeRollbackV1.forkThreadId }
				});
			});
		}
		if (forgeRollbackRestoredV1) {
			forgeNativeThreadRollbackCoverageV1.rollbacks.delete(params.threadId);
			forgeNativeThreadRollbackCoverageV1.snapshotAttemptedThreadIds?.delete(params.threadId);
			embeddedAgentLog.info("forge native thread rollback restored the clean pre-tool fork after a completed tool turn", {
				dirtyThreadId: params.threadId,
				cleanThreadId: forgeRollbackV1.forkThreadId
			});
		}
		return;
	}
	const historyCoveredThrough = (/* @__PURE__ */ new Date()).toISOString();
	const forgeLunaSidecarsV2 = globalThis.__forgeCodexLunaSidecarsV2; // FORGE_CODEX_DUAL_WARM_THREADS_V2_1
	const forgeLunaSidecarKeyV2 = JSON.stringify(params.identity);
	const forgeLunaSidecarV2 = forgeLunaSidecarsV2?.get(forgeLunaSidecarKeyV2);
	if (forgeLunaSidecarV2?.threadId === params.threadId) {
		forgeLunaSidecarsV2.set(forgeLunaSidecarKeyV2, { ...forgeLunaSidecarV2, historyCoveredThrough });
		return;
	}
	// FORGE_CODEX_NATIVE_POSTTURN_HARDCAP_NATIVE_V3
        let forgePostTurnNativeTokensV3;

        try {
                const forgePostTurnRolloutFilesV3 =
                        await listCodexAppServerRolloutFilesForThread(
                                params.agentDir,
                                params.threadId,
                                params.codexHome
                        );

                const forgePostTurnSnapshotsV3 =
                        await Promise.all(
                                forgePostTurnRolloutFilesV3.map(
                                        async (file) =>
                                                readCodexAppServerRolloutTokenSnapshot(file.path)
                                )
                        );

                forgePostTurnNativeTokensV3 =
                        maxFiniteNumber(
                                forgePostTurnSnapshotsV3.map(
                                        (snapshot) => snapshot?.totalTokens
                                )
                        );

        } catch (error) {
                embeddedAgentLog.warn(
                        "forge post-turn native token snapshot failed; preserving current binding",
                        {
                                threadId: params.threadId,
                                error
                        }
                ...
                const forgePostTurnArmedV4 =
                        await params.bindingStore.mutate(params.identity, {
                                ...
                                patch: {
                                        ...
                                        contextEngine: {
                                                schemaVersion: 1,
                                                engineId: "forge-native-rollover",
                                                policyFingerprint: "forge-native-rollover-v4",
                                                projection: {
                                                        schemaVersion: 1,
                                                        mode: "thread_bootstrap",
                                                        epoch: historyCoveredThrough
                                                }
                                        }
                                }
                        }); // FORGE_CODEX_NATIVE_ROLLOVER_BOOTSTRAP_V4

                if (forgePostTurnArmedV4) {
                        embeddedAgentLog.info(
                                "forge native thread reached completed-turn 60k hardcap; fresh rollover armed",
                                {
                                        threadId: params.threadId,
                                        nativeTokens: forgePostTurnNativeTokensV3,
                                        hardcap: 60_000
                                }
                        );
                }
